...
function filterSkills(jobDescription: string, skills: string[]): string[] {
  const keywords = extractKeywords(jobDescription);
  return skills.filter(skill => keywords.includes(skill.toLowerCase()));
};

function filterProjects(jobDescription, projects): string[] {
  const keywords = extractKeywords(jobDescription);
  return projects.filter(project =>
    project.technologies.some(tech => keywords.includes(tech.toLowerCase()))
  );
};
